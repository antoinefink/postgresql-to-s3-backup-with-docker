...
pg_dump -h $DATABASE_IP -p $DATABASE_PORT -U $DATABASE_USERNAME -Fc $DATABASE_NAME | s3cmd  --storage-class=STANDARD_IA --multipart-chunk-size-mb=50 --server-side-encryption put - s3://$DESTINATION/$newname

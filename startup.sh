...
pg_basebackup -h $DATABASE_IP -p $DATABASE_PORT -U $DATABASE_USERNAME -v -P -w --checkpoint=fast --gzip --format=tar -D - | s3cmd --server-side-encryption put - s3://$DESTINATION/$newname
